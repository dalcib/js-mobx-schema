...
export default function convert(mySchema: MySchema): JsonSchema {
  ...
  Object.keys(schema).map((key: string ) => {
    if (Object.prototype.toString.call((schema as any)[key]) === '[object Function]' && (
    ...
    switch (key) {
      case 'properties':
        ...
        Object.keys(schema.properties).map(subKey => {
          (properties as any)[subKey] = convert((schema.properties as any)[subKey])
        })
      ...
      case 'items':
        if (Object.prototype.toString.call(schema.items) === '[object Array]') {
          Object.keys(schema.items).map(subKey => {
            (properties as any)[subKey] = convert((schema.items as any)[subKey])
          })
        } else {
					if (schema.items) {
